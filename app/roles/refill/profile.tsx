...
export default function ProfileScreen() {
  ...
  const [role, setRole] = useState<string>('RefillCoordinator'); // default; will be overridden if present
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          setEmail(u?.email || '');
          setRole(u?.role || 'RefillCoordinator');
  ...
  // ⬅️ Back arrow in header → RefillCoordinator Overview
  useLayoutEffect(() => {
    navigation.setOptions({
      title: 'Profile',
      headerLeft: () => (
        <Pressable
          onPress={() => router.replace('/roles/refill/RefillCoordinatorOverview')}
          style={{ marginLeft: 12 }}
